...
if not st.session_state['logged_in']:
    ...
    if selected == "Профиль":
        ...
        st.markdown("Здесь собраны полезные материалы, темы для разговоров и инструкции, которые помогут сделать ваши сессии эффективными и интересными.")
        
        tab_mentors, tab_speaking, tab_activities = st.tabs(["📚 Гайды для Менторов", "🗣️ Speaking Questions", "🎯 Активности на сессиях"])
        
        with tab_mentors:
            st.subheader("Руководства и советы")
            with st.expander("1. Как провести первую сессию?"):
                st.markdown("""
                - **Знакомство (Icebreaker):** Начните с простых вопросов. Узнайте больше об интересах ученика (менти).
                - **Определение целей:** Спросите, для чего менти изучает казахский язык (работа, учеба, для себя).
                - **Оценка уровня:** Проведите небольшую беседу на базовые темы, чтобы понять текущий уровень владения языком.
                - **План действий:** Обсудите, как часто вы будете встречаться и какие темы покрывать.
                """)
            with st.expander("2. Как давать обратную связь?"):
                st.markdown("""
                - **Метод 'Бутерброда':** Начните с похвалы, затем укажите на ошибки и завершите ободряющими словами.
                - **Исправление ошибок:** Не перебивайте ученика во время речи. Лучше запишите ошибки и обсудите их после завершения мысли.
                - **Позитивный настрой:** Хвалите за попытки использовать новые слова или сложные конструкции.
                """)
                
        with tab_speaking:
            st.subheader("Темы для разговорной практики (A1-B2)")
            with st.expander("👋 Знакомство и о себе (Отлично для первой сессии)"):
                st.markdown("""
                1. **Өзіңіз туралы айтып беріңізші?** (Расскажите о себе?)
                2. **Бос уақытыңызда не істегенді ұнатасыз?** (Что любите делать в свободное время?)
                3. **Қай қалада тұрасыз? Ол қаланың қандай ерекшеліктері бар?** (В каком городе живете? Какие там особенности?)
                """)
            with st.expander("🍲 Еда и Традиции (Тамақ және Дәстүр)"):
                st.markdown("""
                1. **Ең сүйікті ұлттық тағамыңыз қандай?** (Какое ваше любимое национальное блюдо?)
                2. **Қазақтың қандай салт-дәстүрлерін білесіз?** (Какие казахские традиции вы знаете?)
                3. **Наурыз мейрамын қалай атап өтесіз?** (Как вы празднуете Наурыз?)
                """)
            with st.expander("✈️ Путешествия (Саяхат)"):
                st.markdown("""
                1. **Қазақстанның қай жерлерінде болдыңыз?** (Где в Казахстане вы бывали?)
                2. **Сіздің арманыңыздағы саяхат қандай?** (Какое путешествие вашей мечты?)
                3. **Тауға шыққанды ұнатасыз ба, әлде теңізге барғанды ма?** (Любите ходить в горы или на море?)
                """)
                
        with tab_activities:
            st.subheader("Идеи для интерактивных сессий")
            with st.expander("🎭 Ролевые игры (Role-plays)"):
                st.markdown("""
                Смоделируйте реальные ситуации:
                - **В магазине:** Один человек — продавец, другой — покупатель. Цель: купить продукты на казахском языке.
                - **В ресторане:** Заказ еды, диалог с официантом.
                - **На улице:** Как спросить дорогу до ближайшей аптеки или автобусной остановки.
                """)
            with st.expander("📝 Описание картинки (Picture Description)"):
                st.markdown("""
                - Покажите любую фотографию (природа, город, люди).
                - Попросите менти описать, что он видит, используя цвета, предлоги места (алдында, артында, жанында) и действия.
                """)
            with st.expander("🎧 Разбор медиа (Music & Video)"):
                st.markdown("""
                - Включите современную казахскую песню (например, Ninety One, Moldanazar, Dos Mukasan).
                - Попробуйте вместе разобрать текст припева.
                - Посмотрите короткий отрывок из фильма, сериала или интервью и обсудите главную мысль.
                """)
        ...
        if prompt := st.chat_input("Напишите сообщение..."):
            ...
            with st.chat_message("assistant"):
                ...
                # Новый формат: обращаемся к актуальной модели Gemini 3.5 Flash
                API_URL = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key={hf_token}"
